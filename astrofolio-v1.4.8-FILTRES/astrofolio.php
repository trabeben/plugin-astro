 ...
 * Description: Plugin de gestion d'images d'astrophotographie avec métadonnées complètes et système de filtrage avancé
 ...
 * =================================================================
 * Basé sur: v1.4.7-FINAL (base stable validée)
 * Développé le: 5 janvier 2026
 * 
 * 🎯 NOUVELLES FONCTIONNALITÉS :
 * - Système de filtres avancé pour la galerie
 * - Filtrage par type d'objet, télescope, caméra
...
class AstroFolio_Safe {
    ...
    public function init() {
        ...
        add_shortcode('astrofolio_test', array($this, 'test_shortcode'));
        
        // Shortcode pour afficher la version active du plugin
        add_shortcode('astrofolio_version', array($this, 'version_shortcode'));
        
        // =================================================================
        // SHORTCODES DE DEBUG - DÉSACTIVÉS EN VERSION STABLE
        // =================================================================
        /*
        ...
        add_shortcode('astro_test_ajax', array($this, 'test_ajax_shortcode')); // Test AJAX
        */
        ...
        // =================================================================
        // INITIALISATION DES NOUVELLES CLASSES SÉPARÉES (v1.4.8)
        // =================================================================
        
        // Inclure les fichiers des nouvelles classes
        $includes_dir = plugin_dir_path(__FILE__) . 'includes/';
        $public_dir = plugin_dir_path(__FILE__) . 'public/';
        
        // Classes du domaine métier
        if (file_exists($includes_dir . 'class-images.php')) {
            require_once $includes_dir . 'class-images.php';
        }
        
        // Classes publiques pour le frontend
        if (file_exists($public_dir . 'class-public.php')) {
            require_once $public_dir . 'class-public.php';
            new Astro_Public();
        }
        
        if (file_exists($public_dir . 'class-shortcodes.php')) {
            require_once $public_dir . 'class-shortcodes.php';
            new Astro_Shortcodes();
        }
    }
    ...
    private function get_catalog_statistics() {
        ...
        $stats = array();
        $all_catalogs = array('messier', 'ngc', 'ic', 'caldwell', 'sharpless', 'ugc', 'pgc', 'abell');
        
        // Vérifier si les tables existent
        if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) {
            // Si pas de table objects, utiliser les fichiers CSV comme fallback
            foreach ($all_catalogs as $catalog) {
                $stats[$catalog] = 0; // Compter les fichiers CSV pourrait être fait ici
            }
            return $stats;
        }
        
        if ($wpdb->get_var("SHOW TABLES LIKE '$catalog_table'") != $catalog_table) {
            // Si pas de table catalogs, essayer de compter directement depuis astro_objects
            // en supposant qu'il pourrait y avoir une colonne catalog_name
            $object_columns = $wpdb->get_col("SHOW COLUMNS FROM $table_name");
            
            if (in_array('catalog_name', $object_columns)) {
                // Structure ancienne avec catalog_name directement
                foreach ($all_catalogs as $catalog) {
                ...
                // Aucune structure connue, retourner zéro partout
                foreach ($all_catalogs as $catalog) {
                    $stats[$catalog] = 0;
                }
                return $stats;
        ...
        // Les deux tables existent, vérifier quelles colonnes sont disponibles
        $catalog_columns = $wpdb->get_col("SHOW COLUMNS FROM $catalog_table");
        $object_columns = $wpdb->get_col("SHOW COLUMNS FROM $table_name");
        
        // Déterminer la meilleure stratégie en fonction des colonnes disponibles
        if (in_array('name', $catalog_columns) && in_array('catalog_id', $object_columns)) {
            // Structure moderne avec name et catalog_id
            $results = $wpdb->get_results(
                "SELECT c.name, COUNT(o.id) as count 
                 FROM $catalog_table c 
                 LEFT JOIN $table_name o ON c.id = o.catalog_id 
                 GROUP BY c.id, c.name"
            );
            
            foreach ($results as $result) {
                // Déduire le type de catalogue du nom
                $name = strtolower($result->name);
                if (strpos($name, 'messier') !== false) {
                    $stats['messier'] = intval($result->count);
                } elseif (strpos($name, 'ngc') !== false || strpos($name, 'new general') !== false) {
                    $stats['ngc'] = intval($result->count);
                } elseif (strpos($name, 'ic') !== false || strpos($name, 'index') !== false) {
                    $stats['ic'] = intval($result->count);
                } elseif (strpos($name, 'caldwell') !== false) {
                    $stats['caldwell'] = intval($result->count);
                } elseif (strpos($name, 'sharpless') !== false) {
                    $stats['sharpless'] = intval($result->count);
                } elseif (strpos($name, 'ugc') !== false) {
                    $stats['ugc'] = intval($result->count);
                } elseif (strpos($name, 'pgc') !== false) {
                    $stats['pgc'] = intval($result->count);
                } elseif (strpos($name, 'abell') !== false) {
                    $stats['abell'] = intval($result->count);
                }
            }
        } elseif (in_array('catalog_name', $object_columns)) {
            // Structure ancienne avec catalog_name directement dans objects
            foreach ($all_catalogs as $catalog) {
                $count = $wpdb->get_var($wpdb->prepare(
                    "SELECT COUNT(*) FROM $table_name WHERE catalog_name = %s",
                    $catalog
            ...
            // Structure inconnue, essayer de compter tout ce qui existe
            $total_objects = $wpdb->get_var("SELECT COUNT(*) FROM $table_name");
            
            // Répartir approximativement (pas précis mais évite les erreurs)
            foreach ($all_catalogs as $catalog) {
                $stats[$catalog] = 0; // Ou une estimation basique
            }
            
            // Si on a des objets mais ne peut pas les classifier, mettre tout dans NGC par défaut
            if ($total_objects > 0) {
                $stats['ngc'] = intval($total_objects);
            }
        }
        
        // Ajouter les catalogues manquants avec count 0
        foreach ($all_catalogs as $catalog) {
            if (!isset($stats[$catalog])) {
                $stats[$catalog] = 0;
            }
        }
            ...
            <div class="notice notice-info">
                ...
                <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 20px; border-radius: 8px; margin: 20px 0;">
                    ...
                    <form method="post" style="display: inline; margin-right: 10px;">
                        ...
                        ">🔧 CRÉER LES PAGES D'INTÉGRATION</button>
                    </form>
                    
                    <!-- Bouton pour forcer la recréation -->
                    <form method="post" style="display: inline;">
                        <input type="hidden" name="_wpnonce" value="<?php echo wp_create_nonce('astro_public_nonce'); ?>">
                        <input type="hidden" name="action" value="create_integration_pages">
    ...
    private function create_theme_integration_pages() {
        ...
        $detail_page = array(
            ...
            'post_content' => '[astrofolio_version]<h2>TEST DEBUG</h2>[astrofolio_test]<h2>SHORTCODE PRINCIPAL</h2>[astrofolio_image_detail]',
            'post_status' => 'publish',
            'post_type' => 'page',
